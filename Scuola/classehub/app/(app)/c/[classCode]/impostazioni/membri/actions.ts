...
export async function toggleMuteAction(formData: FormData): Promise<void> {
  ...
  if (!target) return;
  setMuted(target.membership.id, !target.membership.muted);
...
export async function rimuoviAction(formData: FormData): Promise<void> {
  ...
  removeMembership(target.membership.id, target.ctx.user.id);

  const parent = getAuthUserById(target.membership.user_id);
  if (parent) {
    await sendEmail({
      to: parent.email,
