...
export async function votaAction(
  ...
  const post = getPostBySlug(ctx.klass.id, slug);
  const poll = post ? getPoll(post.id) : null;
  if (!post || !poll) return { error: it.common.erroreGenerico };
  if (isPollClosed(poll)) return { error: it.sondaggio.erroreChiuso };

  const voterHash = computeVoterHash(ctx.user.id, poll.salt);
  if (hasVoted(post.id, voterHash)) return { error: it.sondaggio.erroreGiaVotato };
  ...
  const ok = castVote({ postId: post.id, optionIds: parsed.data.optionIds, voterHash });
...
export async function toggleArchivioAction(formData: FormData): Promise<void> {
  ...
  const post = getPostBySlug(ctx.klass.id, slug);
  if (post) {
    setArchived(post.id, !post.archived);
    revalidatePath(`/c/${classCode}`);
    revalidatePath(`/c/${classCode}/p/${slug}`);
  }
}
